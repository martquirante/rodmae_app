...
final class PresenceController with WidgetsBindingObserver {
  ...
  Future<void> _writePresence({required bool isOnline}) async {
    ...
    if (AppRuntime.supabaseReady) {
      try {
        ...
        debugPrint('PresenceController._writePresence Supabase error: $e');
      }
    }

    if (AppRuntime.firebaseReady) {
      try {
        final ref = FirebaseDatabase.instance.ref('couples/${AppConfig.coupleId}/presence/$partner');
        await ref.set({
          'isOnline': isOnline,
          'lastSeen': DateTime.now().toUtc().toIso8601String(),
        });
      } catch (e) {
        debugPrint('PresenceController._writePresence Firebase error: $e');
      }
    }
  }
  ...
  Stream<PresenceState> watchPresence(String partnerName) {
    if (!AppRuntime.firebaseReady) {
      return _watchPresenceSupabase(partnerName);
    }
    ...
    void start() {
      try {
        final ref = FirebaseDatabase.instance.ref('couples/${AppConfig.coupleId}/presence/$partnerName');
        firebaseSub = ref.onValue.listen((event) {
          final snapshot = event.snapshot;
          if (snapshot.value == null) {
            if (!ctrl.isClosed) ctrl.add(const PresenceState(isOnline: false));
            return;
          }
          ...
          if (isOnline && lastSeen != null) {
            ...
            if (stale) {
              if (!ctrl.isClosed) ctrl.add(PresenceState(isOnline: false, lastSeen: lastSeen));
              return;
            }
          }
          if (!ctrl.isClosed) ctrl.add(PresenceState(isOnline: isOnline, lastSeen: lastSeen));
        }, onError: (e) {
          debugPrint('watchPresence Firebase error: $e');
        });
      } catch (e) {
        debugPrint('watchPresence Firebase subscription failed: $e');
      }
    }

    ctrl.onListen = start;
    ctrl.onCancel = () => firebaseSub?.cancel();

    return ctrl.stream;
  }

  Stream<PresenceState> _watchPresenceSupabase(String partnerName) {
    final ctrl = StreamController<PresenceState>.broadcast();
    ...
    return StreamBuilder<PresenceState>(
      ...
      builder: (context, snapshot) {
        ...
        final dotColor = isActive
            ...
            : const Color(0xFF9CA3AF); // grey-400 (more visible on dark backgrounds)
            ...
            children: [
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    // Border color matches the card theme to pop out beautifully
                    border: Border.all(
