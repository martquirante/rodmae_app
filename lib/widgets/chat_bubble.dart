...
class ChatBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isFinances = message.message.trim().toLowerCase().startsWith('@finances') ||
                       sender == 'finances';

    final Color bubbleColor;
    if (isFinances) {
      bubbleColor = isDark ? const Color(0xFF78350F) : const Color(0xFFFEF3C7);
    } else if (isAssistant) {
    ...
    if (isFinances) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: RodMaeColors.accentGold,
            width: 1.5,
          ),
          boxShadow: [
            BoxShadow(
              color: RodMaeColors.accentGold.withValues(alpha: 0.35),
              blurRadius: 6,
              spreadRadius: 1,
            ),
          ],
...
class _TextBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isAssistant = message.assistant;
    final isFinances = message.message.trim().toLowerCase().startsWith('@finances') ||
                       message.sender.toLowerCase() == 'finances';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isFinances
              ? RodMaeColors.accentGold.withValues(alpha: 0.3)
              : (isAssistant
                  ? const Color(0xFF10B981).withValues(alpha: 0.3)
                  : Colors.white.withValues(
                      alpha: isDark ? 0.06 : 0.2,
                    )),
          width: (isFinances || isAssistant) ? 1.5 : 1.0,
        ),
        boxShadow: isFinances
            ? [
                BoxShadow(
                  color: isDark
                      ? RodMaeColors.accentGold.withValues(alpha: 0.15)
                      : RodMaeColors.accentGold.withValues(alpha: 0.1),
                  blurRadius: 8,
                  spreadRadius: 1,
                ),
              ]
...
class _VoiceBubbleState extends State<_VoiceBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    final isFinances = widget.message.message.trim().toLowerCase().startsWith('@finances') ||
                       widget.message.sender.toLowerCase() == 'finances';
                ...
                child: Row(
                  children: [
                    _QuickActionChip(
                      ...
                      color: RodMaeColors.accentGold,
                      onTap: () {
                        widget.controller.text = '@finances spent ';
                    ...
                    _QuickActionChip(
                      ...
                      color: RodMaeColors.mint,
                      onTap: () {
                        widget.controller.text = '@finances earned ';
                        widget.controller.selection = TextSelection.fromPosition(
                          TextPosition(offset: widget.controller.text.length),
                        );
                        widget.focusNode?.requestFocus();
                    ...
                    _QuickActionChip(
                      ...
                      onTap: () {
                        widget.controller.text = '@finances view balances';
                        widget.onSend();
                      },
                    ),
                  ],
