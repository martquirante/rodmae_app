...
import 'package:rodmae_app/models/transaction.dart';
...
void main() {
  group('RodMae App Unit Tests', () {
    ...
    test('Transaction model parsing verification', () {
      final map = {
        'id': 'test-1',
        ...
        'amount': 5500.50,
        'type': 'expense',
        'created_by_user_id': 'Rodel',
        'created_at': '2026-06-02T00:00:00Z',
      };

      final entry = Transaction.fromJson(map);

      expect(entry.id, equals('test-1'));
      expect(entry.walletId, equals('shared-wallet'));
      expect(entry.categoryId, equals('Food'));
      expect(entry.amount, equals(5500.50));
      expect(entry.type, equals(TransactionType.expense));
      expect(entry.createdByUserId, equals('Rodel'));
    });
